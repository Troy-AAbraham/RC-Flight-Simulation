...
from math import atan2, asin, pi, exp, sin, cos, sqrt, atan
...
class Mesh:
    def __init__(self,filename,vertexshadername, fragmentshadername,texturename,width,height):
        ...
        self.norm_index = []
        self.model = []
        self.projection_matrix = matrix44.create_perspective_projection_matrix(60.0, width/height,0.1,20000)
        self.position = [0.,0.,0.]
...
class FlightData:
    ...
    def render(self,flight_data):
        #left side data
        g_c = 20
        self.text.draw(-0.9,0.90,"Airspeed", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,0.84,str(round(flight_data["Airspeed"],1))+" fps", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,0.75,"AoA", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,0.69,str(round(flight_data["AoA"],5))+" deg", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,0.60,"Sideslip", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,0.54,str(round(flight_data["Sideslip"],5))+" deg", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,0.45,"Altitude", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,0.39,str(round(flight_data["Altitude"],2))+" ft", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,0.30,"Latitude", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,0.24,str(round(flight_data["Latitude"],6))+" deg", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,0.15,"Longitude", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,0.09,str(round(flight_data["Longitude"],6))+" deg", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,0.0,"Bank", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,-0.06,str(round(flight_data["Bank"],2))+" deg", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,-0.15,"Elevation", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,-0.21,str(round(flight_data["Elevation"],2))+" deg", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,-0.30,"Heading", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,-0.36,str(round(flight_data["Heading"],2))+" deg", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,-0.45,"Gnd Speed", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,-0.51,str(round(flight_data["Gnd Speed"],1))+" fps", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,-0.60,"Gnd Track", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,-0.66,str(round(flight_data["Gnd Track"],2))+" deg", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,-0.75,"Climb", (g_c,g_c,g_c,1))
        self.text.draw(-0.9,-0.81,str(round(flight_data["Climb"],0))+" fpm", (g_c,g_c,g_c,1))


        #right side data
        self.text.draw(0.7,0.90,"Axial G-Force", (g_c,g_c,g_c,1))
        self.text.draw(0.7,0.84,str(round(flight_data["Axial G-Force"],4))+" g's", (g_c,g_c,g_c,1))
        self.text.draw(0.7,0.75,"Side G-Force", (g_c,g_c,g_c,1))
        self.text.draw(0.7,0.69,str(round(flight_data["Side G-Force"],4))+" g's", (g_c,g_c,g_c,1))
        self.text.draw(0.7,0.60,"Normal G-Force", (g_c,g_c,g_c,1))
        self.text.draw(0.7,0.54,str(round(flight_data["Normal G-Force"],4))+" g's", (g_c,g_c,g_c,1))
        self.text.draw(0.7,0.45,"Roll Rate", (g_c,g_c,g_c,1))
        self.text.draw(0.7,0.39,str(round(flight_data["Roll Rate"],2))+" deg/s", (g_c,g_c,g_c,1))
        self.text.draw(0.7,0.30,"Pitch Rate", (g_c,g_c,g_c,1))
        self.text.draw(0.7,0.24,str(round(flight_data["Pitch Rate"],2))+" deg/s", (g_c,g_c,g_c,1))
        self.text.draw(0.7,0.15,"Yaw Rate", (g_c,g_c,g_c,1))
        self.text.draw(0.7,0.09,str(round(flight_data["Yaw Rate"],2))+" deg/s", (g_c,g_c,g_c,1))
        self.text.draw(0.7,0.00,"Throttle", (g_c,g_c,g_c,1))
        self.text.draw(0.7,-0.06,str(round(flight_data["Throttle"],0))+" %", (g_c,g_c,g_c,1))
        self.text.draw(0.7,-0.15,"Elevator", (g_c,g_c,g_c,1))
        self.text.draw(0.7,-0.21,str(round(flight_data["Elevator"],1))+" deg", (g_c,g_c,g_c,1))
        self.text.draw(0.7,-0.30,"Ailerons", (g_c,g_c,g_c,1))
        self.text.draw(0.7,-0.36,str(round(flight_data["Ailerons"],1))+" deg", (g_c,g_c,g_c,1))
        self.text.draw(0.7,-0.45,"Rudder", (g_c,g_c,g_c,1))
        self.text.draw(0.7,-0.51,str(round(flight_data["Rudder"],1))+" deg", (g_c,g_c,g_c,1))
        self.text.draw(0.7,-0.60,"Flaps", (g_c,g_c,g_c,1))
        self.text.draw(0.7,-0.66,str(round(flight_data["Flaps"],1))+" deg", (g_c,g_c,g_c,1))
        self.text.draw(0.7,-0.75,"Time", (g_c,g_c,g_c,1))
        self.text.draw(0.7,-0.81,str(round(flight_data["Time"],1))+" sec", (g_c,g_c,g_c,1))

        #bottom data
        self.text.draw(0.1,-0.75,"Graphics Time Step: " +str(flight_data["Graphics Time Step"])+" sec", (g_c,g_c,g_c,1))
        self.text.draw(-0.6,-0.75,"Physics Time Step: " +str(round(flight_data["Physics Time Step"],6))+" sec", (g_c,g_c,g_c,1))
...
class Camera:
    ...
    def side_view(self, graphics_aircraft):
        """creates view matrix such that camera is to the side of graphics_aircraft. camera location and orientation is tied to graphics_aircraft
    ...
    def stationary_view(self, graphics_aircraft):
        """creates view matrix such that camera is positioned at a stationary point on the field. camera location and orientation is tied to graphics_aircraft
        ...
        """
        to_camera = [0.0,-10.0,-7.]
        quat_orientation = [graphics_aircraft.orientation[3], graphics_aircraft.orientation[0], graphics_aircraft.orientation[1], graphics_aircraft.orientation[2]]
        quat_orientation = [0,0,0,1.]

        graphics_aircraft_to_camera = to_camera

        cam_up = [0.,0.,-5.]
        rotated_cam_up = Body2Fixed(cam_up,quat_orientation)

        self.pos_storage.append(graphics_aircraft_to_camera)
        self.up_storage.append(rotated_cam_up)
        self.target_storage.append(graphics_aircraft.position)
#        self.target_storage.append(np.array([10,0,0]))
